...
export default function Shopping(){
 ...
 const items=useAppStore(state=>state.shopping),toggle=useAppStore(state=>state.toggle),add=useAppStore(state=>state.addItem);
 ...
 const [modal,setModal]=useState(false),[name,setName]=useState('');
 ...
 return <Screen>
  <Page>
   <View style={x.header}><Text accessibilityRole="header" style={x.title}>Danh sách mua sắm</Text><View style={x.headerActions}><IconButton accessibilityLabel="Chia sẻ danh sách" name="share-social-outline"/><IconButton accessibilityLabel="Tùy chọn khác" name="ellipsis-horizontal"/></View></View>
   <CloudStatusNotice status={cloudStatus} error={cloudError} onRetry={retry}/>
   <View style={[x.cols,tablet&&x.row]}>
    <View style={x.list}>{items.length===0?<View accessibilityLiveRegion="polite" style={x.empty}><Text style={x.emptyIcon}>🛒</Text><Text style={x.group}>Danh sách mua sắm đang trống.</Text><Text style={x.muted}>Hãy chọn một món ăn để thêm nguyên liệu.</Text></View>:groups.map(group=><View key={group} style={x.groupBlock}><View style={x.groupHeader}><Text style={x.group}>{group}</Text><View style={x.count}><Text style={x.countText}>{items.filter(item=>item.category===group).length}</Text></View></View>{items.filter(item=>item.category===group).map(item=><Pressable accessibilityRole="checkbox" accessibilityLabel={`${item.name}, ${item.quantity}`} accessibilityState={{checked:item.checked}} onPress={()=>toggleItem(item.id)} key={item.id} style={x.item}><Ionicons name={item.checked?'checkbox':'square-outline'} size={22} color={item.checked?colors.primary:colors.muted}/><Text>{emoji[group]??'🛒'}</Text><Text style={[x.itemName,item.checked&&x.done]}>{item.name}</Text><Text style={x.quantity}>{item.quantity}</Text></Pressable>)}</View>)}</View>
