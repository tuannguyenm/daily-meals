...
export default function Shopping(){
 ...
 const groups=[...new Set(items.map(item=>item.category))],recipeCount=items.filter(item=>item.source==='recipe').length,manualCount=items.length-recipeCount;
 ...
 const addItem=()=>{const trimmed=name.trim();if(trimmed){add({id:`local-${Date.now()}`,name:trimmed,quantity:'1',category:'Gia vị & Khác',checked:false,source:'manual'});sync()}setName('');setModal(false)};
 ...
 return <Screen>
  <Page>
   ...
   {recipeCount>0?<View style={x.generated}><Ionicons name="calendar-outline" size={22} color={colors.primaryDark}/><View style={x.generatedCopy}><Text style={x.generatedTitle}>Đã tổng hợp từ kế hoạch tuần</Text><Text style={x.muted}>{recipeCount} nguyên liệu từ công thức{manualCount?` · ${manualCount} mục thêm tay`:''}</Text></View><Pressable accessibilityRole="button" accessibilityLabel="Xem kế hoạch tuần" onPress={()=>router.push('/tabs/plan')} style={x.planLink}><Text style={x.planLinkText}>Xem kế hoạch</Text></Pressable></View>:null}
