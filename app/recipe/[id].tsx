...
import {persistFavorite,persistMealSelection,persistRecommendationAction,persistShoppingSnapshot} from '../../src/cloud-sync';
...
import {Meal,NutritionInfo,RecipeIngredient,RecipeStep} from '../../src/types';
...
export default function Recipe(){
 ...
 const meal=useMeal(mealId)??getCachedMeal(mealId)??meals[0],readyMade=meal.mealSource==='ready_made',{recipe,source,loading}=useRecipe(mealId,!readyMade);
 ...
 const activePlanDate=useAppStore(state=>state.activePlanDate),selected=useAppStore(state=>state.selectedMeals),selectMealForDate=useAppStore(state=>state.selectMealForDate),completeMeal=useAppStore(state=>state.completeMeal);
 ...
 const shareRecipe=()=>void Share.share({title:meal.title,message:`${meal.title}\n${readyMade?'Mua sẵn':'Tự nấu'} · ${meal.cookingTimeMinutes} phút · ${meal.servings} người\ndailymeals://recipe/${mealId}`});
 const confirmPurchased=()=>{
  if(selected[meal.type]?.id!==meal.id)selectMealForDate(activePlanDate,meal.type,meal);
  completeMeal(meal.id);
  void Promise.all([
   persistMealSelection(familyId,meal.type,{...meal,status:'completed'},activePlanDate),
   persistRecommendationAction(familyId,meal.type,meal.id,'completed'),
  ]).catch(()=>undefined);
 };
 if(readyMade)return <ReadyMadeDetail meal={meal} favorite={favorite} confirmed={selected[meal.type]?.id===meal.id&&selected[meal.type]?.status==='completed'} onFavorite={favoriteMeal} onShare={shareRecipe} onConfirm={confirmPurchased}/>;
...
function ReadyMadeDetail({meal,favorite,confirmed,onFavorite,onShare,onConfirm}:{meal:Meal;favorite:boolean;confirmed:boolean;onFavorite:()=>void;onShare:()=>void;onConfirm:()=>void}){
 const nutrition=nutritionForMeal(meal);
 return <SafeAreaView style={x.safe}>
  <View style={x.header}>
   <Pressable accessibilityRole="button" accessibilityLabel="Quay lại" style={x.back} onPress={()=>router.back()}><Ionicons name="arrow-back" size={23}/></Pressable>
   <Text style={x.headerTitle} numberOfLines={1}>{meal.title}</Text>
   <View style={x.headerIcons}><IconButton accessibilityLabel={favorite?'Bỏ khỏi yêu thích':'Thêm vào yêu thích'} name={favorite?'heart':'heart-outline'} onPress={onFavorite}/><IconButton accessibilityLabel="Chia sẻ món ăn" name="share-social-outline" onPress={onShare}/></View>
  </View>
  <ScrollView contentContainerStyle={x.page}>
   <Image source={meal.image} style={x.readyHero}/>
   <View style={x.readyBadge}><Ionicons name="bag-handle-outline" size={18} color={colors.primaryDark}/><Text style={x.readyBadgeText}>Món mua sẵn · Không cần nấu</Text></View>
   <Text accessibilityRole="header" style={x.title}>{meal.title}</Text>
   <Text style={x.readySummary}>{meal.summary}</Text>
   <View style={x.readyMetrics}>
    <View style={x.readyMetric}><Text style={x.readyMetricValue}>{meal.purchaseTimeMinutes??meal.cookingTimeMinutes} phút</Text><Text style={x.muted}>Thời gian mua</Text></View>
    <View style={x.readyMetric}><Text style={x.readyMetricValue}>~{(meal.pricePerServing??Math.round(meal.estimatedCost/meal.servings)).toLocaleString('vi-VN')}đ</Text><Text style={x.muted}>Mỗi phần</Text></View>
    <View style={x.readyMetric}><Text style={x.readyMetricValue}>{meal.servings} phần</Text><Text style={x.muted}>Cho gia đình</Text></View>
   </View>
   <View style={x.readyCard}><Text style={x.listTitle}>Phù hợp khi</Text><View style={x.readyTags}>{meal.sideDishes.map(label=><View key={label} style={x.readyTag}><Ionicons name="checkmark-circle" size={17} color={colors.primary}/><Text>{label}</Text></View>)}</View><Text style={x.readyNote}>Giá và dinh dưỡng là mức ước tính phổ biến; có thể thay đổi theo quán và khẩu phần thực tế.</Text></View>
   <Nutrition nutrition={nutrition} readyMade/>
  </ScrollView>
  <View style={x.readyFooter}><Button title={confirmed?'✓  Đã mua':'Xác nhận đã mua'} onPress={onConfirm}/><Button title="Đổi món" outline onPress={()=>router.push({pathname:'/meals/alternatives',params:{mealType:meal.type}})}/></View>
 </SafeAreaView>;
}

function Nutrition({nutrition,readyMade=false}:{nutrition:NutritionInfo;readyMade?:boolean}){
...
const x=StyleSheet.create({
 ...
 readyHero:{width:'100%',height:280,borderRadius:20},readyBadge:{alignSelf:'flex-start',flexDirection:'row',alignItems:'center',gap:7,backgroundColor:colors.soft,paddingHorizontal:12,paddingVertical:8,borderRadius:999},readyBadgeText:{fontWeight:'800',color:colors.primaryDark},readySummary:{fontSize:15,color:colors.muted,lineHeight:22},readyMetrics:{flexDirection:'row',flexWrap:'wrap',gap:9},readyMetric:{flex:1,minWidth:105,backgroundColor:'#fff',borderWidth:1,borderColor:colors.border,borderRadius:14,padding:13},readyMetricValue:{fontSize:17,fontWeight:'800',color:colors.primaryDark,marginBottom:3},readyCard:{backgroundColor:'#fff',borderRadius:16,padding:16},readyTags:{gap:10},readyTag:{flexDirection:'row',alignItems:'center',gap:8},readyNote:{fontSize:11,color:colors.muted,lineHeight:17,marginTop:14},readyFooter:{minHeight:76,backgroundColor:'#fff',borderTopWidth:1,borderColor:colors.border,padding:12,flexDirection:'row',gap:8},
});
